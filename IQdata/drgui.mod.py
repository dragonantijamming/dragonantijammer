...
# Create signal variable at file scope
iqsig = None
...
def main():
    ...
    with open('iqdatatest.txt','a+') as f:
        for i in range(0,np.real(iqsig).size):
            f.write(str(np.real(iqsig[i])))
            f.write(',')
            f.write(str(np.imag(iqsig[i])))
            f.write('\n')
